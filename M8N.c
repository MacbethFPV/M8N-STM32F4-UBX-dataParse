...
void M8N_UBX_NAV_POSLLH_Parsing(unsigned char * data, M8N_UBX_NAV_POSLLH* posllh)
{
	posllh->CLASS = data[2];
	posllh->ID = data[3];
	posllh->length = data[4] | data[5]<<8; // shift MSB of the upper address left by 8bits + combine with LSB using OR operation
	...
	// posllh->lonf64 = posllh->lon / 10000000.;
	// posllh->latf64 = posllh->lat / 10000000.;
}